...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <h3 class="text-xs font-bold uppercase tracking-widest text-gray-400">MANAGE TEAMS</h3>
        <a href="{{ route('admin.teams.create') }}" class="inline-flex items-center px-4 py-2 rounded-xl bg-red-600 hover:bg-red-500 border border-red-400/60 text-white font-bold uppercase tracking-widest text-xs shadow-lg shadow-red-900/50 transition-all hover:scale-105">
            + Create Team
        </a>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left">
                ...
                <tbody class="divide-y divide-white/5">
                    @foreach(\App\Models\Team::with('game')->paginate(15) as $team)
                    <tr class="hover:bg-white/5 transition-colors">
                        ...
                        <td class="px-6 py-4">
                            <span class="inline-flex items-center px-2 py-1 rounded-md bg-white/10 text-xs font-bold text-white uppercase tracking-wider">{{ $team->game->name }}</span>
                        </td>
                        <td class="px-6 py-4 text-gray-400 text-sm">{{ $team->region ?? 'Global' }}</td>
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                <form id="delete-team-{{ $team->id }}" action="{{ route('admin.teams.destroy', $team) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" @click="$dispatch('open-confirmation-modal', { title: 'Delete Team', message: 'Are you sure you want to delete {{ $team->name }}?', action: 'delete-team-{{ $team->id }}' })" class="text-gray-400 hover:text-red-500 transition-colors">
